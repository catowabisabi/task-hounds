...
interface StopResponse { ok: boolean; error?: string; }
...
export function BackgroundServerModal({
  ...
  const flow01ErrorStatus = flow01Run?.status === 'failed' || flow01Run?.status === 'error' ? String(flow01Run?.status) : '';
  const flow01ErrorId = flow01Run?.id != null ? flow01Run.id : '';
  const errorSig =
    (loopActionError ? `loop:${loopActionError}` : '') +
    '|' +
    (flow01ErrorStatus ? `flow:${flow01ErrorId}:${flow01ErrorStatus}:${flow01Run?.error ?? ''}` : '');
  ...
  useEffect(() => {
    ...
    if (flow01ErrorStatus) {
      const errTail = flow01Run?.error ? ' — ' + flow01Run.error : '';
      pushToast(`Flow_01 run #${flow01ErrorId} ${flow01ErrorStatus}${errTail}`, 'error');
    }
  }, [open, errorSig, loopActionError, flow01ErrorStatus, flow01ErrorId, flow01Run, pushToast]);

  const hasErrors = !!loopActionError || !!flow01ErrorStatus;
          ...
          <div className='flex-1 overflow-y-auto p-5 space-y-4'>
            {hasErrors && (
              <div className='rounded-lg p-3 space-y-2' style={{ background: 'var(--red-bg)', border: '1px solid var(--red-dim)' }}>
                ...
                {flow01ErrorStatus && flow01Run && (
                  <div className='text-[10px] font-mono break-words' style={{ color: 'var(--text-primary)' }}>
                    <strong>Flow_01 Run #{flow01Run.id}:</strong> {flow01ErrorStatus}
                    {flow01Run.error && <div className='mt-1'>{flow01Run.error}</div>}
                  </div>
                )}
              </div>
            )}
            ...
            {flow01Mode && flow01Run && !hasErrors && (
              <div className='rounded-lg p-3' style={{ background: 'var(--blue-bg)', border: '1px solid var(--blue-dim)' }}>
                ...
                <div className='text-[10px] font-mono' style={{ color: 'var(--text-primary)' }}>
                  Run #{flow01Run.id} — {flow01Run.status}
            ...
            {configInfo && (
              <div className='rounded-lg p-3' style={{ background: 'var(--purple-bg)', border: '1px solid var(--purple-dim)' }}>
                <div className='text-[11px] font-semibold mb-2' style={{ color: 'var(--purple)' }}>OpenCode Config</div>
                <div className='text-[9px] font-mono space-y-1' style={{ color: 'var(--text-secondary)' }}>
                  <div className='break-all'>
                    <strong>XDG_CONFIG_HOME:</strong> {configInfo.xdg_config_home}
                  </div>
                  <div className='break-all'>
