    ...
    const flowSteps: Step[] = [
      ...
      },
      {
        ...
        description: phase === "worker_running"
          ? "executing task"
          : ((phase === "reviewer_running" || done || phase.startsWith("cancelled_after")) ? "done" : "waiting"),
...
export default function App() {
  ...
  const [flow01Run, setFlow01Run] = useState<{ id?: number; status?: string; phase?: string; graph_step?: string; task?: string } | null>(null);
  ...
  const fetchLoop = useCallback(async () => {
    if (flow01Mode) {
      const data = await apiGet<{ runs: Array<{ id: number; status: string; phase?: string; graph_step?: string; task?: string; error?: string }> }>("/api/workflows/flow_01/runs?limit=1").catch(() => ({ runs: [] }));
      const latest = data.runs?.[0] || null;
      setFlow01Run(latest);
      if ((latest?.status === "failed" || latest?.status === "error") && latest?.id != null && reportedFailedFlow01RunRef.current !== latest.id) {
        reportedFailedFlow01RunRef.current = latest.id;
        setLoopActionError("flow_01 " + latest.status + (latest.error ? ": " + latest.error : ""));
      } else if (latest?.status === "running" || latest?.status === "cancelling") {
    ...
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: "var(--bg-base)", color: "var(--text-primary)" }}>
      ...
      <header className="h-[50px] shrink-0 flex items-center px-4 gap-3" style={{ background: "var(--bg-panel)", borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="ml-auto flex items-center gap-1.5">
                ...
                if (flow01Mode) {
                  try {
                    const latestRun = flow01Run?.id
                      ? flow01Run
                      : (await apiGet<{ runs: Array<{ id: number; status: string; phase?: string; graph_step?: string; task?: string }> }>("/api/workflows/flow_01/runs?limit=1").catch(() => ({ runs: [] }))).runs?.[0];
                    if (!latestRun?.id) {
          ...
          {flow01Mode && flow01Run?.id != null && (flow01Run.status === "running" || flow01Run.status === "pausing" || flow01Run.status === "paused") && (
            <button
              onClick={async () => {
                setLoopActionError("");
                try {
                  if (flow01Run.status === "paused" || flow01Run.status === "pausing") {
                    const result = await apiPost<{ ok?: boolean; run_id?: number; status?: string; phase?: string; checkpoint_step?: string }>(`/api/workflows/flow_01/runs/${flow01Run.id}/resume`);
                    setFlow01Run(prev => prev ? { ...prev, status: result.status || "running", phase: result.phase || prev.phase } : prev);
                  } else {
                    const result = await apiPost<{ ok?: boolean; run_id?: number; status?: string; phase?: string }>(`/api/workflows/flow_01/runs/${flow01Run.id}/pause`);
                    setFlow01Run(prev => prev ? { ...prev, status: result.status || "pausing", phase: result.phase || "pausing" } : prev);
                  }
                  fetchLoop();
                } catch (err) {
                  setLoopActionError(err instanceof Error ? err.message : "Pause/Resume failed");
                }
              }}
              className="px-2 py-1 rounded text-[10px] font-semibold transition-colors disabled:opacity-40"
              style={flow01Run.status === "paused"
                ? { background: "var(--green-bg)", color: "var(--green)", border: "1px solid var(--green-dim)" }
                : { background: "var(--amber-bg)", color: "var(--amber)", border: "1px solid var(--amber-dim)" }
              }
              title={flow01Run.status === "paused" || flow01Run.status === "pausing" ? "Resume from last checkpoint" : "Pause before current step"}
            >{flow01Run.status === "paused" || flow01Run.status === "pausing" ? "▶ Resume" : "⏸ Pause"}</button>
          )}
          <button
            onClick={async () => {
              setLoopActionError("");
          ...
          <BackgroundServerButton
            ...
            count={serverCount}
            hasErrors={!!loopActionError || flow01Run?.status === "failed" || flow01Run?.status === "error"}
