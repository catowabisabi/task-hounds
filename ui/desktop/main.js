...
// Use an API endpoint supported by both FastAPI and the legacy server.
...
const MAX_WAIT_MS      = 30000; // Maximum startup wait, in ms.
...
// Backend startup. Supports the packaged PyInstaller server.
function startPythonServer() {
  // Prefer the PyInstaller server executable from extra-bin when available.
  ...
  if (usePackaged) {
    ...
    console.log(`[Electron] Starting packaged server: "${PACKAGED_EXE}" --port ${SERVER_PORT}`);
    ...
    console.log(`[Electron] starting Python server: ${PYTHON_CMD} -m ${SERVER_PACKAGE} --port ${SERVER_PORT}`);
    console.log(`[Electron] cwd: ${APP_ROOT}`);

    pythonProcess = spawn(PYTHON_CMD, ['-m', SERVER_PACKAGE, '--port', String(SERVER_PORT)], {
      cwd: APP_ROOT,
      env,
      stdio: ['ignore', 'pipe', 'pipe'],
      windowsHide: true,
    ...
    pythonProcess.on('error', (err) => {
      ...
      dialog.showErrorBox(
        ...
        `Could not start the backend server.\nPlease confirm Python 3.9+ is installed.\n\nError: ${err.message}`
      );
      app.quit();
    });
  ...
  try {
    ...
    dialog.showErrorBox(
      ...
      isPythonCrash
        ...
        : `The backend Python server did not start within ${MAX_WAIT_MS / 1000} seconds.\n\n` +
          'Please confirm:\n' +
